...
function severity(child-2tion: string): Severity {
  return SEVERITY_MAP[child-2tion] ?? 'warning';
}

// ─── Validation Types ────────────────────────────────────────────

interface DataPointSourceIssue {
  child-2tion: 'missing-source' | 'wrong-source-type' | 'multiple-sources';
  nodeId: string;
  label: string;
  source: string;
...
interface TransformIssue {
  child-2tion: 'no-inputs' | 'no-outputs';
  nodeId: string;
  label: string;
...
interface ComponentReferenceIssue {
  child-2tion: 'invalid-capture-reference' | 'invalid-display-reference' | 'component-no-datapoints';
  nodeId: string;
  label: string;
  invalidRefs: string[];
  severity: Severity;
...
interface CircularDependency {
  child-2tion: 'circular-dependency';
  cycle: string[];
  labels: string[];
  severity: Severity;
}
...
interface WorkflowIssue {
  child-2tion: 'empty-workflow' | 'insufficient-workflow-members' | 'workflow-no-outputs' | 'dangling-member-reference' | 'nested-workflow';
  nodeId: string;
  label: string;
  detail?: string;
  severity: Severity;
...
interface TbdIssue {
  child-2tion: 'datapoint-tbd-type' | 'table-column-tbd';
  nodeId: string;
  label: string;
  detail?: string;
  severity: Severity;
...
function validateComponentReferences(nodes: any[], edges: any[]): ComponentReferenceIssue[] {
  ...
  for (const component of components) {
    ...
    if (invalidCaptures.length > 0) {
      issues.push({
        child-2tion: 'invalid-capture-reference',
        nodeId: component.id,
        label,
    ...
    if (invalidDisplays.length > 0) {
      issues.push({
        child-2tion: 'invalid-display-reference',
        nodeId: component.id,
        label,
    ...
    if (!hasDataPointEdge) {
      issues.push({
        child-2tion: 'component-no-datapoints',
        nodeId: component.id,
        label,
        invalidRefs: [],
...
function validateWorkflows(nodes: any[], edges: any[]): WorkflowIssue[] {
  ...
  for (const n of nodes) {
    ...
    if (data.members.length < 2) {
      issues.push({
        child-2tion: 'insufficient-workflow-members',
        nodeId: n.id,
        label,
    ...
    if (outgoingEdges.length === 0) {
      issues.push({
        child-2tion: 'workflow-no-outputs',
        nodeId: n.id,
        label,
    ...
    for (const m of data.members) {
      if (m.transformId && !nodeIdSet.has(m.transformId)) {
        issues.push({
          child-2tion: 'dangling-member-reference',
          nodeId: n.id,
          label,
...
function validateTbdIssues(nodes: any[]): TbdIssue[] {
  ...
  for (const n of nodes) {
    if (n.type === 'datapoint' && n.data?.type === 'tbd') {
      issues.push({
        child-2tion: 'datapoint-tbd-type',
        nodeId: n.id,
        label: n.data?.label ?? 'Untitled',
...
export async function handleValidateProject(args: z.infer<typeof validateProjectSchema>) {
  ...
  if (dataPointSourceIssues.length > 0) {
    ...
    for (const issue of dataPointSourceIssues) {
      lines.push(`- [${issue.severity}] **${issue.label}** (${issue.source}): ${issue.child-2tion.replace(/-/g, ' ')}`);
  ...
  if (transformIssues.length > 0) {
    ...
    for (const issue of transformIssues) {
      lines.push(`- [${issue.severity}] **${issue.label}** (${issue.transformType}): ${issue.child-2tion.replace(/-/g, ' ')}`);
    }
  }
  ...
  if (workflowIssues.length > 0) {
    ...
    for (const issue of workflowIssues) {
      lines.push(`- [${issue.severity}] **${issue.label}**: ${issue.child-2tion.replace(/-/g, ' ')}${issue.detail ? ` — ${issue.detail}` : ''}`);
    }
  }
  ...
  if (componentReferenceIssues.length > 0) {
    ...
    for (const issue of componentReferenceIssues) {
      lines.push(`- [${issue.severity}] **${issue.label}**: ${issue.child-2tion.replace(/-/g, ' ')}`);
  ...
  if (tbdIssues.length > 0) {
    ...
    for (const issue of tbdIssues) {
      lines.push(`- [${issue.severity}] **${issue.label}**: ${issue.detail ?? issue.child-2tion.replace(/-/g, ' ')}`);
    }
  }
